...
# UART 16550 IPのデータシート

## ボーレート

ボーレートは2つのレジスタを使ってソフトウェアで構成できます。

- Divisor Latch
- Pre-Scaler Divisition

両レジスタは最大ボーレート（システムクロック周波数 / 16）をユーザが選択可能な係数で除算します。
Divisor Latchは16550標準の16ビットレジスタであり、DLLとDLMという2つの8ビットロケーションから
アクセスされます。1から65536の除算因子はプログラム可能です。Pre-Scaler Divisionレジスタは
標準的ではありません（ただし、モジュールの互換性を全く制限するものではありません）。このレジスタは
システムクロックが最も一般的なUARTクロック周波数の倍数である場合に有用な、追加の除算係数（1〜16）を
提供する4ビットのレジスタです。この場合、共通クロックと合わせて使用される同じDLL/DLM値が便利な
プリスケーリングを加えるだけで使用できるでしょう。

まとめるとボーレートは次のように定義できます。

```bash
BR = f_clk / (16 * (PSD + 1) * DL)
```

**訳注**: f_clk = 24MHz, BR = 115200, PSDはなし; DL = f / 16 * BR = 24000000 / 16 * 115200 = 13

これらのレジスタにアクセスするには、ライン制御レジスタのDLABビットをセットする必要があります。
